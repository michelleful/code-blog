...
So here's the rough plan for the blogposts (if there's a link it's up; if not, please be patient!)

* Getting data from OpenStreetMap and opening it in GeoPandas
* Manipulating geodata in GeoPandas
* Building a basic classifier in scikit-learn
* How to efficiently add features to a classifier using Pipelines
